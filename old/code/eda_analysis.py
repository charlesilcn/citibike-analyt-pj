...
def analyze_time_trends(df, save_dir):
    ...
    plt.title("2024年各月骑行量趋势", fontsize=14) #年份名称
    plt.xlabel("月份", fontsize=12)
    plt.ylabel("骑行次数（万次）", fontsize=12)
    plt.xticks(range(12), [f"{i}月" for i in range(1, 13)])
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    plt.savefig(os.path.join(save_dir, "2024_monthly_trips.png"), dpi=300, bbox_inches="tight")
    plt.close()
    print("✅ 月度骑行量图表已保存")
    ...
    plt.title("2024年工作日vs周末每小时骑行量对比", fontsize=14) #年份名称
    plt.xlabel("小时（0-23）", fontsize=12)
    plt.ylabel("骑行次数", fontsize=12)
    plt.xticks(range(0, 24, 2))
    plt.legend(fontsize=11)
    plt.grid(linestyle="--", alpha=0.5)
    plt.savefig(os.path.join(save_dir, "2024_weekday_weekend_hour.png"), dpi=300, bbox_inches="tight")
    plt.close()
...
def analyze_user_behavior(df, save_dir):
    ...
    plt.title("2024年用户类型占比（会员vs非会员）", fontsize=14) #年份名称
    plt.axis("equal")
    plt.savefig(os.path.join(save_dir, "2024_user_type_pie.png"), dpi=300, bbox_inches="tight") #年份名称
    plt.close()
    print("✅ 用户类型占比饼图已保存")
    ...
    plt.title("2024年会员vs非会员骑行时长对比（≤60分钟）", fontsize=14) #年份名称
    plt.xlabel("用户类型", fontsize=12)
    plt.ylabel("骑行时长（分钟）", fontsize=12)
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    plt.savefig(os.path.join(save_dir, "2024_user_duration_box.png"), dpi=300, bbox_inches="tight") #年份名称
    plt.close()
...
def analyze_station_popularity(df, save_dir):
    ...
    plt.title("2024年热门出发站点 Top 10", fontsize=14) #年份名称
    plt.xlabel("骑行出发次数", fontsize=12)
    plt.ylabel("站点名称", fontsize=12)
    plt.grid(axis="x", linestyle="--", alpha=0.5)
    plt.savefig(os.path.join(save_dir, "2024_top_start_stations.png"), dpi=300, bbox_inches="tight") #年份名称
    plt.close()
    print("✅ 热门出发站点图表已保存")

def main():
    featured_data_path = "../processed_data/2024_featured.csv"
    charts_save_dir = "../results/charts/2024 charts"
